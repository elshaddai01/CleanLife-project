...
export async function getStoredRole(): Promise<'client' | 'collector' | null> {
  return AsyncStorage.getItem(ROLE_KEY) as Promise<
...
async function request<T>(
  ...
  try {
    ...
    throw new ApiError(
      0,
      `Cannot reach the CleanLife server at ${API_BASE}. Check the server address and Wi-Fi connection.`
    );
  }
...
export function deriveStatus(
  pr: BackendPickupRequest,
  hasProofOfWork: boolean
): PickupStatus {
  if (pr.routing_status === 'completed') {
    return 'disposal_confirmed';
  }

  if (hasProofOfWork) {
    return 'disposal_submitted';
  }

  if (pr.cash_collected_at || pr.momo_confirmed_at) {
    return 'pickup_complete';
  }

  if (pr.collector_arrived_at) {
    return 'arrived';
  }

  if (pr.routing_status === 'assigned') {
    return 'on_the_way';
  }

  if (pr.routing_status === 'broadcast_public') {
    return 'broadcast_public';
  }

  if (pr.routing_status === 'searching_corporate') {
    return 'admin_hold';
  }
...
export const pickupApi = {
  ...
  listAvailable() {
    return request<BackendPickupRequest[]>(
  ...
  submitProofOfWork(
    ...
    }>(`/pickup-requests/${requestId}/proof-of-work`, {
      method: 'POST',
      body: JSON.stringify(params),
    });
  },
...
export const walletApi = {
  ...
  topup(amount: number, description?: string) {
    return request<{ id: number; new_balance: string }>(
      '/wallet/topup',
      {
        method: 'POST',
        body: JSON.stringify({
